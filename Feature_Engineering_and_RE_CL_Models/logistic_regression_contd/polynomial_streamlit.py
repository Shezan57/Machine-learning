...
st.sidebar.markdown("# Polynomial Regression Classifier")

dataset = st.sidebar.selectbox("Select Dataset", ['Binary', 'Multiclass'])
penalty = st.sidebar.selectbox("Regularization", ['l1', 'l2', 'elasticnet', 'none'])
c_input = float(st.sidebar.text_input("C", value=1.0))
solver = st.sidebar.selectbox("Solver", ['newton-cg', 'lbfgs', 'liblinear', 'sag', 'saga'])
max_iter = int(st.sidebar.text_input("Max Iterations", value=100))
